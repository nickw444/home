...
class MediaPlayerLight(hass.Hass):
    def initialize(self):
        ...
        current_entity_picture = self.get_state(
            self.args[CONF_MEDIA_PLAYER_ENTITY], attribute="entity_picture"
        )
    ...
    def on_entity_picture_changed(self, entity_picture):
        ...
        if state != "playing":
            self.log(
                f"media player is not playing ({state}). Ignoring state change",
                level="INFO",
            )
            return
        ...
        if enable_source is not None and source != enable_source:
            self.log(
                f"the current source ({source}) is not set to the enable source "
                f" ({enable_source}). Ignoring state change",
                level="INFO",
            )
            return
